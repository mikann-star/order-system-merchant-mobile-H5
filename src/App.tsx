...
function OrdersPage({ orders, setDetail, confirmOrder }: { orders: Order[]; setDetail: (d: Detail) => void; confirmOrder: (id: string) => void }) {
  ...
  const [confirmationFilter, setConfirmationFilter] = useState<'待确认' | '已确认' | '全部'>('待确认')
  const [paymentFilter, setPaymentFilter] = useState<'全部' | '已支付' | '待支付'>('全部')
  ...
  return <><Header title="订单管理" /><section className="order-page-heading"><div><h2>订单管理</h2><p>查看和管理门店订单</p></div></section><section className="order-filter-panel"><label>日期范围<div className="date-range"><input type="date" value={startDate} onChange={event => setStartDate(event.target.value)} /><span>至</span><input type="date" min={startDate} value={endDate} onChange={event => setEndDate(event.target.value)} /></div></label><div className="order-filter-actions"><label>确认状态<select value={confirmationFilter} onChange={event => setConfirmationFilter(event.target.value as '待确认' | '已确认' | '全部')}><option>待确认</option><option>已确认</option><option>全部</option></select></label><label>支付状态<select value={paymentFilter} onChange={event => setPaymentFilter(event.target.value as '全部' | '已支付' | '待支付')}><option>全部</option><option>已支付</option><option>待支付</option></select></label><button className="confirm-all-button" disabled={!pendingCount} onClick={() => setConfirmAll(true)}>全部确认{pendingCount ? ` (${pendingCount})` : ''}</button></div></section><div className="order-list-title"><b>订单列表</b><span>共 {list.length} 条</span></div>
    <section className="list order-list">{list.map(order => <article className="list-card order-list-card" key={order.id} onClick={() => setDetail({ type: 'order', value: order })}><div className="order-id-row"><b>{order.id}</b><Status>{order.status === '待确认' ? '未确认' : '已确认'}</Status></div><div className="order-table-row"><strong>{order.table === '外卖' ? '外卖订单' : `${order.table} 桌`}</strong><span>{order.source} · {order.time}</span></div><p className="order-dish-preview">{order.items.length} 种菜品　{order.items.map(item => item.name).join('、')}</p><div className="card-bottom"><span><em className={order.status === '已完成' ? 'paid' : 'unpaid'}>{order.status === '已完成' ? '已支付' : '待支付'}</em><i>无退款</i></span><strong>¥{orderAmount(order).toFixed(2)}</strong></div>{order.status === '待确认' && <button className="confirm-order" onClick={event => { event.stopPropagation(); confirmOrder(order.id) }}>✓ 确认订单</button>}</article>)}</section>{confirmAll && <div className="modal-backdrop"><section className="modal confirm-all-modal"><h2>确认全部订单</h2><p>当前有 {pendingCount} 条待确认订单，是否全部确认？</p><div><button onClick={() => setConfirmAll(false)}>取消</button><button className="primary" onClick={confirmEveryOrder}>确定</button></div></section></div>}</>
}

function ReservationsPage({ reservations, setDetail, updateReservation, onBack }: { reservations: Reservation[]; setDetail: (d: Detail) => void; updateReservation: (id: string, status: Reservation['status']) => void; onBack?: () => void }) {
  const [filter, setFilter] = useState('全部')
  const list = reservations.filter(r => filter === '全部' || r.status === filter)
  return <><Header title="预订管理" back={onBack} right={<button className="text-action">今日⌄</button>} /><div className="mini-stats"><div><strong>12</strong><span>今日预订</span></div><div><strong>5</strong><span>待就餐</span></div><div><strong>6</strong><span>已到店</span></div></div>
    <div className="chips padded">{['全部', '待就餐', '已到店', '已取消'].map(x => <button className={filter === x ? 'active' : ''} onClick={() => setFilter(x)} key={x}>{x}</button>)}</div>
    <section className="list">{list.map(r => <article className="list-card" key={r.id} onClick={() => setDetail({ type: 'reservation', value: r })}><div className="card-top"><b>{r.guest} · {r.people} 人</b><Status>{r.status}</Status></div><p>{r.time}　·　{r.table} 桌</p><div className="card-bottom"><span>{r.phone}</span>{r.status === '待就餐' && <button className="inline-primary" onClick={e => { e.stopPropagation(); updateReservation(r.id, '已到店') }}>确认到店</button>}</div></article>)}</section><button className="floating" onClick={() => setDetail({ type: 'module', value: '新增预订' })}>＋ 新增预订</button></>
}

const modules = [ ['◷', '预订管理'], ['🍽', '菜品管理'], ['✉', '客户反馈'], ['▥', '财务统计'], ['⌂', '店铺设置'], ['✦', 'AI 端设置'], ['⚙', '系统设置'] ]
function MorePage({ setDetail, onOpenReservations, onOpenFeedback, pendingFeedbackCount }: { setDetail: (d: Detail) => void; onOpenReservations: () => void; onOpenFeedback: () => void; pendingFeedbackCount: number }) { return <><Header title="更多功能" /><section className="module-grid">{modules.map(([icon, name]) => <button key={name} onClick={() => name === '预订管理' ? onOpenReservations() : name === '客户反馈' ? onOpenFeedback() : setDetail({ type: 'module', value: name })}><i>{icon}</i><span>{name}</span>{name === '客户反馈' && pendingFeedbackCount > 0 && <b>{pendingFeedbackCount}</b>}</button>)}</section><section className="more-tip"><b>今日经营提醒</b><p>有 {pendingFeedbackCount} 条客户反馈等待处理，请及时查看。</p></section></> }
...
function ServiceRequestsPage({ items, setItems, setDetail, notify }: { items: ServiceRequest[]; setItems: (items: ServiceRequest[]) => void; setDetail: (detail: Detail) => void; notify: (message: string) => void }) {
  ...
  const visible = items.filter(item => filter === '全部' || item.status === filter)
  const handle = (id: string) => { setItems(items.map(item => item.id === id ? { ...item, status: '已处理' } : item)); notify('服务请求已处理') }
  const handleAll = () => { const count = items.filter(item => item.status === '待处理').length; if (!count) return; setItems(items.map(item => ({ ...item, status: '已处理' }))); notify(`已处理 ${count} 条服务请求`) }
  const deleteProcessed = () => { const count = items.filter(item => item.status === '已处理').length; if (!count) return; if (window.confirm(`确定删除 ${count} 条已处理服务请求吗？`)) { setItems(items.filter(item => item.status !== '已处理')); notify('已删除已处理服务请求') } }
  const pendingCount = items.filter(item => item.status === '待处理').length
  const processedCount = items.filter(item => item.status === '已处理').length
  return <>
    <Header title="服务请求" />
    <section className="service-toolbar"><button className="primary" disabled={!pendingCount} onClick={handleAll}>✓ 全部处理{pendingCount ? ` (${pendingCount})` : ''}</button><button disabled={!processedCount} onClick={deleteProcessed}>删除已处理</button></section>
    <div className="chips padded service-filters">{['全部', '待处理', '已处理'].map(item => <button className={filter === item ? 'active' : ''} onClick={() => { setFilter(item); setExpandedId(null) }} key={item}>{item}</button>)}</div>
    <div className="service-list-title"><b>请求列表</b><span>共 {visible.length} 条</span></div>
    <section className="list service-list" onClick={() => setExpandedId(null)}>
      {visible.length ? visible.map(item => <article className="list-card service-card" key={item.id} onClick={() => setDetail({ type: 'service', value: item })}>
        <div className="service-card-top"><div><b>{item.table} 桌 · {item.source}</b><time>{item.time}</time></div><button className={`request-handle ${item.status === '已处理' ? 'done' : ''}`} disabled={item.status === '已处理'} onClick={event => { event.stopPropagation(); handle(item.id) }}>{item.status === '已处理' ? '已处理' : '✓ 确认处理'}</button></div>
        <p className={expandedId === item.id ? 'expanded' : ''} onClick={event => { event.stopPropagation(); setExpandedId(expandedId === item.id ? null : item.id) }}>{item.category && `【${item.category}】`}{item.content}</p>
      </article>) : <div className="service-empty"><i>✓</i><b>{filter === '待处理' ? '暂无待处理服务请求' : filter === '已处理' ? '暂无已处理服务请求' : '暂无服务请求'}</b><span>新的服务请求会在这里显示。</span></div>}
    </section>
  </>
}
...
function CustomerFeedbackPage({ items, setItems, setDetail, notify, onBack }: { items: ServiceRequest[]; setItems: (items: ServiceRequest[]) => void; setDetail: (detail: Detail) => void; notify: (message: string) => void; onBack: () => void }) {
  ...
  const handleAll = () => { const count = items.filter(item => item.status === '待处理').length; if (!count) return; setItems(items.map(item => ({ ...item, status: '已处理' }))); notify(`已处理 ${count} 条客户反馈`) }
  const deleteProcessed = () => { const count = items.filter(item => item.status === '已处理').length; if (!count) return; if (window.confirm(`确定删除 ${count} 条已处理客户反馈吗？`)) { setItems(items.filter(item => item.status !== '已处理')); notify('已删除已处理客户反馈') } }
  const pendingCount = items.filter(item => item.status === '待处理').length
  const processedCount = items.filter(item => item.status === '已处理').length
  return <><Header title="客户反馈" back={onBack} /><section className="service-toolbar"><button className="primary" disabled={!pendingCount} onClick={handleAll}>✓ 全部处理{pendingCount ? ` (${pendingCount})` : ''}</button><button disabled={!processedCount} onClick={deleteProcessed}>删除已处理</button></section><div className="chips padded service-filters">{['全部', '待处理', '已处理'].map(item => <button className={filter === item ? 'active' : ''} onClick={() => { setFilter(item); setExpandedId(null) }} key={item}>{item}</button>)}</div><div className="service-list-title"><b>反馈列表</b><span>共 {visible.length} 条</span></div><section className="list service-list" onClick={() => setExpandedId(null)}>{visible.length ? visible.map(item => <article className="list-card service-card" key={item.id} onClick={() => setDetail({ type: 'feedback', value: item })}><div className="service-card-top"><div><b>{item.table} 桌 · {item.source}</b><time>{item.time}</time></div><button className={`request-handle ${item.status === '已处理' ? 'done' : ''}`} disabled={item.status === '已处理'} onClick={event => { event.stopPropagation(); handle(item.id) }}>{item.status === '已处理' ? '已处理' : '✓ 确认处理'}</button></div><p className={expandedId === item.id ? 'expanded' : ''} onClick={event => { event.stopPropagation(); setExpandedId(expandedId === item.id ? null : item.id) }}>{item.content}</p></article>) : <div className="service-empty"><i>✓</i><b>{filter === '待处理' ? '暂无待处理客户反馈' : filter === '已处理' ? '暂无已处理客户反馈' : '暂无客户反馈'}</b><span>新的客户反馈会在这里显示。</span></div>}</section></>
}
...
function PasswordModal({ onClose, notify }: { onClose: () => void; notify: (message: string) => void }) {
  const [oldPassword, setOldPassword] = useState(''); const [password, setPassword] = useState(''); const [confirm, setConfirm] = useState(''); const [error, setError] = useState('')
  ...
  const submit = () => { if (oldPassword !== demoAccount.password) { setError('原密码错误，请重新输入'); return }; if (!passwordOk) { setError('请输入符合要求的新密码'); return }; if (!matches) { setError('两次输入的密码不一致'); return }; notify('密码修改成功'); onClose() }
  return <div className="modal-backdrop"><section className="modal account-modal"><header><h2>修改登录密码</h2><button className="modal-close" aria-label="关闭" onClick={onClose}>×</button></header><label>原密码<input type="password" value={oldPassword} onChange={event => { setOldPassword(event.target.value); setError('') }} placeholder="请输入原密码" /></label><label>新密码<input type="password" value={password} onChange={event => { setPassword(event.target.value); setError('') }} placeholder="请输入新密码" /></label><div className="password-rule"><span className={lengthOk ? 'valid' : ''}>{lengthOk ? '✓' : '○'} 长度限制：8-20 位字符</span><span className={kinds >= 3 ? 'valid' : ''}>{kinds >= 3 ? '✓' : '○'} 组合要求：大写字母、小写字母、数字、常用特殊符号(!@#$%^&amp;*_)中，至少包含三类</span></div><label>确认新密码<input type="password" value={confirm} onChange={event => { setConfirm(event.target.value); setError('') }} placeholder="请再次输入新密码" /></label>{confirm && !matches && <small className="field-error">两次输入的密码不一致</small>}{confirm && matches && !passwordOk && <small className="field-error">请输入符合要求的新密码</small>}{error && <small className="field-error">{error}</small>}<div><button onClick={onClose}>取消</button><button className="primary" disabled={!passwordOk || !matches} onClick={submit}>确定修改</button></div></section></div>
}
...
function DetailPage({ detail, close, openModule, openTablePage, openEditTable, openTableModule, returnToTable, openTable, transferTable, updateTableDetails, updateTable, confirmOrder, updateReservation, updateServiceRequest, updateFeedback, tables, orders, dishes, categories, onDishesChange, onCategoriesChange, addTable, notify }: { detail: Exclude<Detail, null>; close: () => void; openModule: (name: string) => void; openTablePage: (table: Table) => void; openEditTable: (table: Table) => void; openTableModule: (table: Table, name: string) => void; returnToTable: (table: Table) => void; openTable: (table: Table, diners: number, startedAt: string) => void; transferTable: (source: Table, target: Table) => boolean; updateTableDetails: (table: Table) => void; updateTable: (id: string, s: TableStatus) => void; confirmOrder: (id: string) => void; updateReservation: (id: string, s: Reservation['status']) => void; updateServiceRequest: (id: string, note: string) => void; updateFeedback: (id: string, note: string) => void; tables: Table[]; orders: Order[]; dishes: Dish[]; categories: DishCategory[]; onDishesChange: (dishes: Dish[]) => void; onCategoriesChange: (categories: DishCategory[]) => void; addTable: (table: Table) => void; notify: (message: string, duration?: number) => void }) {
  ...
  if (detail.type === 'order') {
    ...
    const refundStatus = fullyRefunded ? '已退款' : partiallyRefunded ? '部分退款' : '无退款'
