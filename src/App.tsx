...
type ReceiptPrintType = 'customer' | 'kitchen'
const receiptLabel = (type: ReceiptPrintType) => type === 'customer' ? '顾客小票' : '后厨小票'
const enqueueReceiptPrintJobs = (orderIds: string[], types: ReceiptPrintType[] = ['customer', 'kitchen']) => {
  const stored = JSON.parse(sessionStorage.getItem('fkm-receipt-print-queue') ?? '[]')
  const queue = Array.isArray(stored) ? stored : []
  const jobs = orderIds.flatMap(orderId => types.map(type => ({ orderId, type, createdAt: Date.now() })))
  sessionStorage.setItem('fkm-receipt-print-queue', JSON.stringify([...queue, ...jobs]))
}
const receiptPrintFailureMessage = (error: unknown) => error instanceof DOMException && error.name === 'QuotaExceededError' ? '打印队列存储空间不足，请清理后重试' : error instanceof DOMException && error.name === 'SecurityError' ? '打印服务不可用，请检查浏览器权限' : error instanceof Error && error.message ? `打印失败：${error.message}` : '打印失败，请重试'
...
function OrdersPage({ orders, setDetail, confirmOrder, confirmAllOrders, notify }: { orders: Order[]; setDetail: (d: Detail) => void; confirmOrder: (id: string) => void; confirmAllOrders: (ids: string[]) => void; notify: (message: string, duration?: number) => void }) {
...
function TableOrderCore({ orders, discount, onCheckout, notify }: { orders: Order[]; discount: TableDiscount; onCheckout: () => void; notify: (message: string) => void }) {
  ...
  const [printingCheckout, setPrintingCheckout] = useState(false)
  ...
  const printCheckoutReceipt = () => {
    if (printingCheckout) return
    setPrintingCheckout(true)
    window.setTimeout(() => {
      try {
        const stored = JSON.parse(sessionStorage.getItem('fkm-receipt-print-queue') ?? '[]')
        const queue = Array.isArray(stored) ? stored : []
        queue.push({ type: 'checkout', table: orders[0]?.table ?? '', orderIds: orders.map(order => order.id), createdAt: Date.now() })
        sessionStorage.setItem('fkm-receipt-print-queue', JSON.stringify(queue))
        notify('已打印结账小票')
      } catch (error) { notify(receiptPrintFailureMessage(error)) }
      setPrintingCheckout(false)
    }, 650)
  }
  const confirmRefund = () => {
...
function DetailPage({ detail, close, region, onRegionChange, openModule, openTablePage, openEditTable, openTableModule, returnToTable, openTable, transferTable, updateTableDetails, updateTable, confirmOrder, submitTableOrder, updateReservation, updateServiceRequest, updateFeedback, tables, orders, dishes, categories, onDishesChange, onCategoriesChange, addTable, addReservation, notify }: { detail: Exclude<Detail, null>; close: () => void; region: string; onRegionChange: (region: string) => void; openModule: (name: string) => void; openTablePage: (table: Table) => void; openEditTable: (table: Table) => void; openTableModule: (table: Table, name: string) => void; returnToTable: (table: Table) => void; openTable: (table: Table, diners: number, startedAt: string) => void; transferTable: (source: Table, target: Table) => boolean; updateTableDetails: (table: Table) => void; updateTable: (id: string, s: TableStatus) => void; confirmOrder: (id: string) => void; submitTableOrder: (table: Table, items: PendingOrderItem[]) => void; updateReservation: (id: string, s: Reservation['status']) => void; updateServiceRequest: (id: string, note: string) => void; updateFeedback: (id: string, note: string) => void; tables: Table[]; orders: Order[]; dishes: Dish[]; categories: DishCategory[]; onDishesChange: (dishes: Dish[]) => void; onCategoriesChange: (categories: DishCategory[]) => void; addTable: (table: Table) => void; addReservation: (reservation: Reservation) => void; notify: (message: string, duration?: number) => void }) {
  ...
  const [printingReceipt, setPrintingReceipt] = useState<ReceiptPrintType | null>(null)
  ...
  if (detail.type === 'module' && detail.value === '财务统计') return <FinanceStatsPage orders={orders} dishes={dishes} onBack={close} />
...
export default function App() {
  ...
  const confirmOrder = (id: string) => { setOrders(current => current.map(o => o.id === id ? { ...o, status: '制作中' } : o)); setDetail(current => current?.type === 'order' && current.value.id === id ? { ...current, value: { ...current.value, status: '制作中' } } : current); try { enqueueReceiptPrintJobs([id]) } catch (error) { window.setTimeout(() => notify(receiptPrintFailureMessage(error), 1000), 1000) }; notify('订单已确认', 1000) }
  const confirmAllOrders = (ids: string[]) => { const targetIds = new Set(ids); enqueueReceiptPrintJobs(ids); setOrders(current => current.map(order => targetIds.has(order.id) && order.status === '待确认' ? { ...order, status: '制作中' } : order)); notify(`已确认 ${ids.length} 条订单，顾客及后厨小票已按下单时间正序发送打印`) }
